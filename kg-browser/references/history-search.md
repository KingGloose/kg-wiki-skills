...
解决一类真实场景：**用户记得"之前看过一篇讲 XX 的文章"，但想不起链接在哪。**
...
**1. 主动扩展关键词。** 用户说"那篇讲知识库的知乎"，别只搜"知识库"——
...
**3. 拿到候选后和用户确认，别自己挑。** 标题相似的很多，让用户认。

**4. 时间是有力线索。** 用户说"上周看的"→ `--days 10`；"很久之前"→ 不限时间但加大 `--limit`。
