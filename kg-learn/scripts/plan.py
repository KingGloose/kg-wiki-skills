...
LEARN_DIR = None  # 随 VAULT 在 main() 里初始化
...
def main() -> int:
    ...
    for _sp in (p_new, p_show, p_done, p_note, p_sess, p_arch):
        _sp.add_argument("--vault", default=None, help="知识库路径")
    ap.add_argument("--vault", default=None, help="知识库路径")

    args = ap.parse_args()

    global VAULT, LEARN_DIR
    try:
        VAULT = find_vault(__file__, explicit=getattr(args, "vault", None))
