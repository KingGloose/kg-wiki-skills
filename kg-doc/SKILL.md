...
把用户手上的文档转成干净 Markdown，AI 解析后按 `AGENTS.md` 沉淀进 `wiki/`。
...
```bash
cd "$KG/kg-doc" && source "$KG/.venv/bin/activate"
```
...
- **敏感文档**（简历、合同、内部材料、含个人信息的）：解析可以做，但沉淀进 wiki 前先问用户；不要把个人隐私信息摘录到对话或笔记里。
