...
# 知识库定位（新用户最容易漏这步）
step "知识库定位"
if OUT=$("$VENV_PY" -c "from media_to_text import find_vault; print(find_vault())" 2>&1); then
  ok "已找到知识库: $OUT"
else
  warn "尚未配置知识库位置——skills 不知道往哪写"
  echo "    请任选一种方式："
  echo "      export KG_VAULT=/path/to/your-vault          （加进 ~/.zshrc 或 ~/.bashrc）"
...
后续步骤：
  ...
  1.5) 配置知识库： export KG_VAULT=/path/to/your-vault  （见上方提示）
