...
```bash
cd kg-wiki-skills/kg-browser && bash scripts/connect-chrome.sh
```
...
| 问题页 `.../question/q` | **无限滚动 + 多回答**，先和用户确认要看几条，滚动加载后逐条取 |
...
转换后**先和用户讨论**：这篇讲了什么、值不值得沉淀。别默认全都要存。
